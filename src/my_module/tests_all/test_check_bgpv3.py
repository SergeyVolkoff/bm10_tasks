...
def test_check_enable_bgpv3():
    assert check_enable_bgpv3()==True, (
        "BGP disable"
    )

def test_check_redistr_kernel():
    assert check_redistr_kernel()==True, (
        "Redistribute_kernel - disable"
    )

def test_check_redistr_connected():
    assert check_redistr_connected()==True, (
        "Redistribute_connected - disable"
    )

def test_check_redistr_static():
    assert check_redistr_static()==True, (
        "Redistribute_static - disable"
    )

def test_check_route10_bgpv3():
    assert check_route10_bgpv3()==True, (
        "*** Route to the network 200.1.10.0/24 is not available! ***"
    )

def test_check_route20_bgpv3():
    assert check_route20_bgpv3()==True, (
        "*** Route to the network 200.1.20.0/24 is not available! ***"
    )
...
def test_check_ping_loopback_neighbor(ip_test):
    assert check_ping_interf(ip_for_ping=f"{ip_test}")==True, (
        f"*** IP {ip_test} unavaileble now***"
    )
